...
💡提示：当前 ***`s905w`*** 系列的盒子只支持使用 `5.15.y` 内核，其他型号的盒子可任选内核版本使用。当前 ***`s905`*** 的盒子只能在 `TF/SD/USB` 中使用，其他型号的盒子同时支持写入 `EMMC` 中使用。每个盒子的 dtb 和 u-boot 请查阅[说明](https://github.com/ophub/amlogic-s9xxx-armbian/blob/main/build-armbian/armbian-docs/config_correspondence_of_amlogic_s9xxx_tv_box.md)。
...
| -s | Size | 对固件的 ROOTFS 分区大小进行设置，默认大小为 960MiB, 固件大小必须大于 256M. 例如： `-s 960` |
...
- `sudo ./make -d -b s905x3 -k 5.10.100 -s 960` : 使用默认配置，指定一个内核，一个型号进行打包，固件大小设定为960MiB。
...
- `sudo ./make -d -s 960 -k 5.10.100` : 使用默认配置，设置固件大小为 960MiB, 并指定内核为 5.10.100 ，对全部型号电视盒子进行打包。
