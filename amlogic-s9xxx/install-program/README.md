...
reboot
```
You can also put the upgrade file in the `/mnt/mmcblk*p4/` directory, the `s9xxx-update.sh` script will automatically find the upgrade file from the `/mnt/mmcblk*p4/` and `/tmp/upload/` directories, and the supported formats are `*.img, *.img.xz, *.img.gz`
