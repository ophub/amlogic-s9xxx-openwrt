...
rebuild_firmware() {
    ...
    my_packages="\
        ...
        perl-http-date perlbase-file perlbase-getopt perlbase-time perlbase-unicode  \
        perlbase-utf8 pigz ppp ppp-mod-pppoe proto-bonding pv rename resize2fs runc  \
        subversion-client subversion-libs tar tini ttyd tune2fs uclient-fetch uhttpd  \
        uhttpd-mod-ubus unzip uqmi usb-modeswitch uuidgen wget-ssl whereis which wpa-cli  \
        wpad-basic wwan xfs-fsck xfs-mkfs xz xz-utils ziptool zoneinfo-asia  \
        zoneinfo-core zstd  \
        \
